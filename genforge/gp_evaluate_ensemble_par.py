...
def ensemble_task(args):
    ...
    for id_comb in range(len(comb_ens1)):
        ...
        prob_comb_val = prob_comb_val1[id_comb]
        prob_comb_ts = prob_comb_ts1[id_comb]
...
def gp_evaluate_ensemble_par(gp):
    ...
    for ens in range(num_ens_comb):
        ...
        for id_pop in range(len(ens_comb[ens])):
            ...
            prob_comb_val[ens].append(copy.deepcopy(prob_val[id_pop][ens_comb[ens][id_pop]])) if ybin_val is not None else None
            prob_comb_ts[ens].append(copy.deepcopy(prob_ts[id_pop][ens_comb[ens][id_pop]])) if ybin_ts is not None else None
