...
from .softmaxlossgrad import softmaxlossgrad

def gradient_descent_softmax(gene_output_str2, gene_outputs_val2, y_binary_tr, y_binary_val,
                             initial_learning_rate, epsilon, lambda_L1, lambda_L2,
                             tolgrad, max_iteration):
    """
    Gradient Descent Algorithm to find gene weights using softmax.
    """
    softmax_loss_grad_fun = softmaxlossgrad(gene_output_str2)
    ...
    softmax_fun = softmax(gene_outputs_val2)
    ...
    grad_fun = lambda x, y, w, l1, l2: (1.0 / num_data_tr) * (softmax_loss_grad_fun(x, y, w) + l1_derivative(l1, w) + l2_derivative(l2, w))
    ...
    for itr in range(max_iteration):
        ...
        # Loss
        loss = logloss(softmax_fun(gene_outputs_val2, w_temp), y_binary_val)
    ...
    plt.close('all')
    return w_final, grad_final, loss_val_final
